...
public class SentimentNetwork {
    ...
    private DoubleMatrix layer_0;
    private DoubleMatrix weights_0_1;
    private DoubleMatrix weights_1_2;

    public SentimentNetwork(Dictionary dict){
        Random.seed(1);
        this.dict = dict;
    ...
    private void initializeNetwork() {
        ...
        this.layer_0 = DoubleMatrix.zeros(1, this.inputNodes);
    }

    public void updateInputLayer(String review) {
        this.layer_0 = DoubleMatrix.zeros(1, this.inputNodes);
        for (String word: review.split(" ")) {
            int index = dict.getIndex(word);
            if (index > 0)
//                this.layer_0.put(0, index, this.layer_0.get(0, index) + 1); Counts how many times the word appears, lower accuracy
                this.layer_0.put(0, index, 1);
        }
    }
    ...
    public void train(ArrayList<String> trainingReviews, ArrayList<String> trainingLabels) {
        ...
        for(int i = 0; i < trainingReviews.size(); i ++) {
            String review = trainingReviews.get(i);
            String label = trainingLabels.get(i);

            this.updateInputLayer(review);

            DoubleMatrix layer_1 = this.layer_0.mmul(this.weights_0_1);
            DoubleMatrix layer_2 = this.sigmoid(layer_1.mmul(this.weights_1_2));
            ...
            this.weights_1_2 = this.weights_1_2.sub(layer_1.transpose().mmul(layer_2_delta).mmul(this.learningRate));
            this.weights_0_1 = this.weights_0_1.sub(this.layer_0.transpose().mmul(layer_1_delta).mmul(this.learningRate));
    ...
    public double predict(String review) {
        this.updateInputLayer(review.toLowerCase());
